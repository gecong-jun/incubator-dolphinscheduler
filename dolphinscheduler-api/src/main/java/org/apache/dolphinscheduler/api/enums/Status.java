...
public enum Status {
    ...
    CREATE_ROLE_ERROR(100900, "create role error", "创建角色错误"),
    DELETE_ROLE_BY_ID_ERROR(100901, "delete role by id error", "删除角色错误"),
    ROLE_NOT_EXIST(100902, "role {0} not exists", "角色[{0}]不存在"),
    UPDATE_ROLE_ERROR(100903, "update user error", "更新用户错误"),
    ROLE_NAME_EXIST(100904, "role name already exists", "角色名已存在"),
    QUERY_ROLE_LIST_PAGING_ERROR(10091, "query role list paging error", "分页查询角色列表错误"),
    QUERY_TASK_INSTANCE_LOG_ERROR(100905, "view task instance log error", "查询任务实例日志错误"),
