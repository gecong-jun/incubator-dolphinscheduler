  ...
  <m-popover
    ...
    <template slot="content">
      <div class="create-user-model">
        <m-list-box-f>
          <template slot="name"><strong>*</strong>{{$t('Role Name')}}</template>
          <template slot="content">
            <el-input
                    type="input"
                    v-model="roleName"
        ...
        <m-list-box-f style="line-height: 38px;">
          ...
          <template slot="content">
            <el-radio-group v-model="roleState" size="small">
  ...
  export default {
    ...
    methods: {
      _ok () {
        ...
        this.store.dispatch(`role/${this.item ? 'updateRole' : 'createRole'}`, param).then(res => {
      ...
      _verification () {
        ...
        let roleNameLength = this.roleName.length
        // user name
        if (roleNameLength < 3 || roleNameLength > 39) {
          this.$message.warning(`${i18n.$t('Role name length is between 3 and 39')}`)
          return false
        }
        // if (!this.roleName.replace(/\s*/g, '')) {
        //   this.$message.warning(`${i18n.$t('Please enter role name')}`)
        //   return false
        // }
        // password
    ...
    created () {
      ...
      // } else {
        if (this.item) {
          ...
          this.email = this.item.email
          this.phone = this.item.phone
          // this.roleState = this.isDisable + '' || '1'
          this.roleState = this.item.isDisable ? 1:0
          // if (this.item.tenantId) {
          //   this.tenantId = this.item.tenantId
          // }
