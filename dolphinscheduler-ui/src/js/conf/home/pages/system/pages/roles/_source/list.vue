  ...
  <div class="list-model user-list-model">
    <div class="table-box">
      <el-table :data="list" size="mini" style="width: 100%">
        ...
        <el-table-column prop="roleName" :label="$t('Role Name')"></el-table-column>
        <el-table-column prop="roleCode" :label="$t('Role Code')"></el-table-column>
        <el-table-column :label="$t('State')" width="60">
          <template slot-scope="scope">
            {{scope.row.isDisable === false ? `${$t('Enable')}` : `${$t('Disable')}`}}
          </template>
        </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapActions('role', ['deleteRole', 'getAuthList', 'grantAuthorization', 'getResourceList']),
      _delete (item, i) {
        this.deleteRole({
          id: item.id
        }).then(res => {
